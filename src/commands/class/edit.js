...
module.exports = {
    name: "#class-edit",
    alias: ["#class-edit", "#ce"],
    description: "Edit class info. Format: #class-edit [Field], [Value]",
    execute: async (bot, from, sender, args, msg, text) => {
        ...
        const rawContent = text.replace("#class-edit", "").trim();
        const parts = rawContent.split(",").map(p => p.trim());

        if (parts.length < 2) {
            return bot.sock.sendMessage(from, {
                text: `⚠️ *Format Edit Kelas*\n\nContoh:\n\`#class-edit nama, Sistem Informasi B\`\n\`#class-edit deskripsi, Grup Resmi Angkatan 24\``
            });
        }
